...
router.post('/getlist/order_id', async function (req, res) {
  ...
  let your_order= await order_detailsModel.findOne({order_id:req.body.order_id}).populate([{ path: "user_id", select: ["first_name", "middle_name", "last_name", "user_email", "user_phone", "user_address"] },{path: "store", select: ["name","phoneno","email","location","type","address","code"] },
 ...
 router.post("/payment_initiate_one", async function (req, res) {
   try {
    req.body.amount = 1;
       ...
       let credentials = paytm_credentials();
       let orderid = uuidv4();
       let callbackurl = "http://ec2-44-208-166-141.compute-1.amazonaws.com/api/order_details/callbackurl";
       var paytmParams = {};
       paytmParams.body = {
         "requestType": "Payment",
         "mid" : credentials.mid,
